...
class NapeSpace extends FlxBasic
{
	...
	override public function draw():Void
	{
		...
		{
			shapeDebug.drawLine(ray.origin, Vec2.weak(ray.origin.x + ray.direction.x * ray.length, ray.origin.y + ray.direction.y * ray.length), ray.color);
		}
		...
		sprite.scaleX = zoom * FlxG.scaleMode.scale.x;
		sprite.scaleY = zoom * FlxG.scaleMode.scale.y;
		
		sprite.x = -FlxG.camera.scroll.x * zoom * FlxG.scaleMode.scale.x;
		sprite.y = -FlxG.camera.scroll.y * zoom * FlxG.scaleMode.scale.y;
		#end
	}
}